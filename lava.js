...
//display is created by giving it a parent element to which it should append itself and a level object
...
//append returns appended element to create wrapper element and store it in wrap property in a singe comment

var scale = 20; //scale var gives number of pixels that single unit takes up
...
//display level when game moves to next level or resets
...
//function expects time difference as an argument and draws a single frame
//when frame function returns false, animation stops
//converts time steps to seconds, easier to think about than milliseconds
...
//runLevel function takes a Level object, a constructor for a display a function
//it displays the level (in document.body) and lets the user play 
...
//when player dies current level is restarted
//level is completed move on to the next
//function takes array of level plans and a display constructor

//function argument is used to arrange things to happen in the future
//wrapping these actions in functions gives us way to store them as a value
//so they can be called at the right moment, asynchronous programming
